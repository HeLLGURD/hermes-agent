...
def fake_mcp_server(populated_sessions_dir, mock_session_db, monkeypatch):
    import hermes_cli.mcp_serve as mcp_serve
...
class TestContentExtraction:
    def test_text(self):
        from hermes_cli.mcp_serve import _extract_message_content
...
class TestAttachmentExtraction:
    def test_image_url_block(self):
        from hermes_cli.mcp_serve import _extract_attachments
        msg = {"content": [
...
class TestEventBridge:
    def test_create(self):
        from hermes_cli.mcp_serve import EventBridge
        b = EventBridge()
        assert b._cursor == 0
        assert b._queue == []

    def test_enqueue_and_poll(self):
        from hermes_cli.mcp_serve import EventBridge, QueueEvent
        b = EventBridge()
    ...
    def test_queue_limit(self):
        from hermes_cli.mcp_serve import EventBridge, QueueEvent, QUEUE_LIMIT
        b = EventBridge()
...
class TestE2EEventsPoll:
    ...
    def test_poll_with_events(self, mcp_server_e2e, _event_loop):
        from hermes_cli.mcp_serve import QueueEvent
        server, bridge = mcp_server_e2e
...
class TestEventBridgePollE2E:
    ...
    def test_poll_interval_is_200ms(self):
        ...
        from hermes_cli.mcp_serve import POLL_INTERVAL
